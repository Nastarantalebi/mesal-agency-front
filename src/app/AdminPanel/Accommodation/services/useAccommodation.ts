...
export const useAccommodation = (AccommodationId?: number, currentAccommodationPage?: number, searchInput?: string, onClickSearch?: boolean) => {
  ...
  const url =  `${accommodation_url}${AccommodationId}/`
    ...
    >({
      key: [accommodation_key, String(currentAccommodationPage), searchInput!, "lkasdfn"],
      url: `${accommodation_url}${searchInput && onClickSearch === true ? `?name__contains=${searchInput}` : `?page=${currentAccommodationPage}`}`,
    });
