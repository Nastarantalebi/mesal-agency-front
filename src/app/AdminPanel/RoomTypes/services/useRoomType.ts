...
export const useRoomType = (
  ...
  const { mutateAsync: putRoomType, isPending: ispendingPut } =
    usePutData<TCreateRoomType>({
      key,
      url,
    });

  const { mutateAsync: postRoomType, isPending: ispendingPost } =
    usePostData<TCreateRoomType>({
      key,
      url,
    });
...
export const useRoomTypeList = (
  ...
  const getRoomTypeList = useGetData<TPaginatedResponse<TRoomTypeResponse>>({
    key: ["RoomTypes", String(AccommodationId), searchInput!],
    url: `${admin_accommodation_url}${AccommodationId}/room_types/${searchInput && `?name__contains=${searchInput}`}`,
  });
...
export const useRoomTypeImg = (AccommodationId: number, RoomTypeId: number) => {
  ...
  const getImgs = useGetData<TPaginatedResponse<TRoomTypeImageResponse>>({
    key,
    url,
    enabled: !!RoomTypeId,
  });
...
export const useRoomTypeFeatures = (
  AccommodationId: number,
  RoomTypeId: number,
  currentRoomTypeFeaturePage: number,
) => {
  ...
  const getFeatures = useGetData<TPaginatedResponse<TFeatureResponse>>({
  ...
  const postRoomTypeFeatures = usePostData<TCRoomTypeFeature>({
    key,
    url,
  });
...
export const useRooms = (AccommodationId: number, RoomTypeId: number) => {
  ...
  const { mutateAsync: postRoom, isPending: ispendingPost } =
    usePostData<TCRoomTypesRoom>({
      key,
      url,
    });
...
export const useRoomList = (
  ...
  const getRooms = useGetData<TPaginatedResponse<TRoomTypeRoomResponse>>({
    key: ["RoomType-rooms", String(RoomTypeId), searchInput!],
    url: `${admin_accommodation_url}${AccommodationId}/room_types/${RoomTypeId}/rooms/${searchInput && `?name__contains=${searchInput}`}`,
  });

  const deleteRoom = useDeleteData({
    key: ["RoomType-rooms", String(RoomTypeId)],
...
export const useRoomTypeBed = (AccommodationId: number, RoomTypeId: number) => {
  ...
  const getRoomTypeBeds = useGetData<TRoomTypeBedResponse>({
    key,
    url,
    enabled: !!RoomTypeId,
  });
...
export const useRoomTypePrice = (
  ...
  const postRoomTypePrices = usePostData<TCRoomTypePrices>({
    key,
    url,
  });
